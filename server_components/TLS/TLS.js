...
export function TLSHandshake(data, socket){
    ...
    */
    ...
    // using BigInt for all bitshift-operations, to avoid signed integers
    ...
    let length = 0n;
    length = (BigInt(data[3]) << 8n) | BigInt(data[4]); // data/payload/message/fragment length (2 bytes)
    ...
    switch (type){
        ...
        case 22:    // Handshake
            ...
            let handshakeMessageLength = 0n;
            handshakeMessageLength = (BigInt(data[6]) << 16n) | (BigInt(data[7]) << 8n) | BigInt(data[8]); // length of handshake message
            ...
            gmt_unix_time: 0n,                  // the clientside time, for random structure. (4 bytes, clientside time in seconds)
            ...
            let ciphersuite_length = 0n;         // the length of the ciphersuite (2 bytes)
            ...
            let extensions_total_length = 0n;   // the length of the extensions field (2 bytes)
            ...
            switch(handshakeType){
                ...
                case 1:
                    ...
                    randomStructure.gmt_unix_time = ((BigInt(data[11]) << 24n)| (BigInt(data[12]) << 16n)| (BigInt(data[13]) << 8n)| (BigInt(data[14])));
                    ...
                    ciphersuite_length = (BigInt(data[(44 + offset)]) << 8n) | BigInt(data[45 + offset]);
                    ...
                    if((position + 1) !== 0){
                        ...
                        extensions_total_length = (BigInt(data[position]) << 8n) | BigInt(data[(position+1)]);
                        position += 2;  // update position
                        extensions = data.slice(position, position + Number(extensions_total_length));
